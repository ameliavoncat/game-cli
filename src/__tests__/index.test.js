...
command:
  name: testcmd3
  description: generated by tests (3)
  usage: testcmd3 [options] <command>
  _inactive: true
`,

          source: 'export const invoke = "NOT A FUNCTION"'
        },

      }

      Object.keys(this.testCommandsToWrite).forEach(cmdName => {
        fs.writeFileSync(this.cmdConfigFilePath(cmdName), this.testCommandsToWrite[cmdName].config)
        fs.writeFileSync(this.cmdSourceFilePath(cmdName), this.testCommandsToWrite[cmdName].source)
      })
    ...
    it('ignores commands with no invoke() function', function () {
      expect(this.requiredCommands).not.to.have.property('testcmd1')
      expect(this.requiredCommands).to.have.property('testcmd2')
      expect(this.requiredCommands).not.to.have.property('testcmd3')
    })
  })
})

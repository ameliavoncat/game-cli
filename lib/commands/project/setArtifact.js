...
function invokeSetProjectArtifactURLAPI(lgJWT, projectDescriptor, url) {
  var mutation = {
    ...
    variables: { projectDescriptor: projectDescriptor, url: url }
  };
...
function setProjectArtifactURL(args, notify, options) {
  ...
  var projectName = _args$_[0];
  var url = _args$_[1];

  var projectDescriptor = projectName.replace(/^#/, '');
  return invokeSetProjectArtifactURLAPI(lgJWT, projectDescriptor, url).then(function (project) {
    if (project.id) {

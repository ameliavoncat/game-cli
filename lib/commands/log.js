...
function formatSubject(subject) {
  ...
  var subjectParts = subject.map(function (part) {
    return part.handle;
  });
  var oxfordComma = subject.length > 2 ? ',' : '';
  var subjectString = subjectParts.slice(0, subjectParts.length - 2).join(', ');
  subjectString = '' + subjectString + oxfordComma + ' and ' + subjectParts[subjectParts.length - 1];
...
function formatQuestion(question, _ref) {
  ...
  var subjectString = formatSubject(question.subject);
